...
with tab_overview:
    if has_data:
        ...
        # Excel uses AVERAGE(H_block) where block = 4 PTUs of the same hour
        ...
        # Total ID position forecast (C) = ID trades + (Wind eff - Wind DA)
        #   + (Nowcast eff - Nowcast DA) + (LargePV eff - LargePV DA) + DA position
        ...
        # Assemble final display dataframe in Excel column order
        ...
        # Bold-style columns via highlighting
        ...
        def highlight_columns(row):
            ...
            for i, col in enumerate(row.index):
                ...
                if col in overwrite_cols:
                    styles[i] += "; background-color: #FFF2CC" if styles[i] else "background-color: #FFF2CC"
            return styles
